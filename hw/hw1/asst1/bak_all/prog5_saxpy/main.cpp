...
extern void saxpyStream(int N, float a, float* X, float* Y, float* result);
extern void saxpyStreamThread(int N, float a, float* X, float* Y, float* result);
...
//    float* result = new float[N];
    float* arrayX = (float*)memalign(32, N * sizeof(float));
    float* arrayY = (float*)memalign(32, N * sizeof(float));
    float* result = (float*)memalign(32, N * sizeof(float));
...
// printf("[saxpy serial]:\t\t[%.3f] ms\t[%.3f] GB/s\t[%.3f] GFLOPS\n",
    ...
    //
    // Run the stream intrinsic implementation
    //
    double minStream = 1e30;
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        saxpyStream(N, scale, arrayX, arrayY, result);
        double endTime = CycleTimer::currentSeconds();
        minStream = std::min(minStream, endTime - startTime);
    }

    printf("[saxpy stream]:\t[%.3f] ms\t[%.3f] GB/s\t[%.3f] GFLOPS\n",
           minStream * 1000,
           toBW(TOTAL_BYTES, minStream),
           toGFLOPS(TOTAL_FLOPS, minStream));

    printf("\t\t\t\t(%.2fx speedup from use of stream)\n", minISPC/minStream);


    //
    // Run the stream intrinsic thread implementation
    //
    double minStreamThread = 1e30;
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        saxpyStreamThread(N, scale, arrayX, arrayY, result);
        double endTime = CycleTimer::currentSeconds();
        minStreamThread = std::min(minStreamThread, endTime - startTime);
    }

    printf("[saxpy stream thread]:\t[%.3f] ms\t[%.3f] GB/s\t[%.3f] GFLOPS\n",
           minStreamThread * 1000,
           toBW(TOTAL_BYTES, minStreamThread),
           toGFLOPS(TOTAL_FLOPS, minStreamThread));

    printf("\t\t\t\t(%.2fx speedup from use of stream threads)\n", minISPC/minStreamThread);
    delete[] arrayX;
    delete[] arrayY;
    delete[] result;
